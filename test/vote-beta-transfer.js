...
describe('VoteTokenBeta transfers', async function () {
    beforeEach(async () => {
        [
            alice,
            bob,
        ...
        voteTokenBetaInstance = await VoteTokenBetaDev.connect(owner).deploy();
        await voteTokenBetaInstance.deployed();
    ...
    it('transfers', async () => {
        await voteTokenBetaInstance.mint(alice.address, 1000000);
        let instance = voteTokenBetaInstance.connect(alice);
