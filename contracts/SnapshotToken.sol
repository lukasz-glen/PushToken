 ...
 * A snapshot is taken when {_snaphost()} is called.
 ...
 */
contract SnapshotToken is BaseToken {
     ...
     *
     * An attack with a large number of snapshots is less harmful.
     */
    function _snapshot() internal virtual returns (uint256) {
        uint256 currentSnapshotId = _lastSnapshotId + 1;
        require(currentSnapshotId < 2**48-1, "SnapshotToken: too big snapshot id");
        _lastSnapshotId = currentSnapshotId;
    ...
    function _balanceOfAtSnapshot(address account, uint256 snapshotId, uint256 snapshotKey) internal view returns (uint256 balance) {
        require(
            ...
            "SnapshotToken: snapshot does not belong to caller"
        );
        require(
            snapshotId >= ((snapshotKey & 0x0000000000000000000000000000000000000000FFFFFFFFFFFF000000000000) >> 48),
            "SnapshotToken: snapshot incompatible with snapshot key, >="
        );
        require(
    ...
    function _balanceOfAtSnapshot(address account, uint256 snapshotId) internal view returns (uint256 balance) {
        ...
        require(
            ...
            "SnapshotToken: snapshotId is outdated"
        );
        // maybe it is unnecessary, target should recognize snapshotId
        require(snapshotId <= _lastSnapshotId, "SnapshotToken: snapshot id in the future");
    ...
    function _reclaimGas(uint256 snapshotKey) internal {
        ...
        require(
            ...
            "SnapshotToken: cannot delete current balance"
        );
    ...
    function _transfer(
        ...
        uint256 nextSnapshotId = _lastSnapshotId + 1;
        require(senderBalance >= amount, "SnapshotToken: transfer amount exceeds balance");
        unchecked {
    ...
    function _mint(address account, uint256 amount) internal virtual {
        require(account != address(0), "SnapshotToken: mint to the zero address");

        _beforeTokenTransfer(address(0), account, amount);

        uint256 totalSupply_ = _totalSupply + amount;
        require(totalSupply_ < 2**208, "SnapshotToken: maximal total supply exceeded");
    ...
    function _burn(address account, uint256 amount) internal virtual {
        require(account != address(0), "SnapshotToken: burn from the zero address");
